...
runme:
  ...
  session:
    ...
    updated: 2026-04-25 14:53:14-04:00
---
...
* NAN should be NAN in all instances
* `typeOf()` will return a number
* used to define a number that is not a real number <br> `'ABC'/4 = NAN`
* any value compared to NAN is false <br> `NAN !== 1, NAN !== false` <br> even `NAN === NAN` will return `false`
* `isNAN()` is a built-in function to check for NAN <br> `isNAN(NAN) \\true` <br> `isNAN(Number()"A") ``true`
* if the variable is not equal to itself then and only then will it actually be NAN <br> `NAN a!==a`